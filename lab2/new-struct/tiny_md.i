# 0 "tiny_md.c"
# 1 "/users/cp2025/cp2025_093/mejor_optimizacion2/tiny_md//"
# 0 "<built-in>"
# 0 "<command-line>"
...
# 51 "tiny_md.c"
        ...
        Etail = tail * (double)500;
        Ptail = tail * Rho;

        int i = 0;
        sf = cbrt(Rhob / Rho);
        for (int k = 0; k < 3 * 500; k++) {
            particles.rf[gr(k)] *= sf;
        }


        forces(particles.rf, &Epot, &Pres, &Temp, Rho, cell_V, cell_L);

        for (i = 1; i < 500; i++) {

            velocity_verlet(particles.rf, particles.v, &Epot, &Ekin, &Pres, &Temp, Rho, cell_V, cell_L);
        ...
        for (i = 500; i < 2000; i++) {
            ...
            if (i % 10 == 0) {
                ...
                fprintf(file_xyz, "%d\n\n", 500);
                for (int k = 0; k < 3 * 500; k += 3) {
                    fprintf(file_xyz, "Ar %e %e %e\n", particles.rf[gr(k + 0)], particles.rf[gr(k + 1)], particles.rf[gr(k + 2)]);
                }
            }
