    ...
    *epot = 0.0;
    ...
    for (int i = 0; i < 3 * (N - 1); i += 3) {
        ...
        for (int j = i + 3; j < 3 * N; j += 3) {
            ...
            __m256d r1_sq = _mm256_mul_pd(r_xyz, r_xyz);
            
            //Limpiar ultima pos
            __m256d zero = _mm256_setzero_pd();  // [0.0, 0.0, 0.0, 0.0]
            r1_sq = _mm256_blend_pd(r1_sq, zero, 0b1000);
            
            // Paso 1: horizontal sum parcial: x+y, z+0
            __m256d r1_hadd = _mm256_hadd_pd(r1_sq, r1_sq);  // [x+y, z+0, x+y, z+0]
            
            // Paso 2 SIMD: extraer x+y (posición 0 del hadd) y z (posición 2 del original)
            
            __m128d r1_xy = _mm256_castpd256_pd128(r1_hadd);        // x1² + y1²
            __m128d r1_z  = _mm256_extractf128_pd(r1_hadd, 1);         // z1²
            
            
            // Sumar xy + z para cada vector
            __m128d r1_sum = _mm_add_sd(r1_xy, r1_z);  // solo suma el primer double
            
            double rx = r_xyz[0];  
            double ry = r_xyz[1];  
            double rz = r_xyz[2];
            double rij2 = _mm_cvtsd_f64(r1_sum); 
