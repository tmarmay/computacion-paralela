    ...
    const double* temp, const double rho, const double V, const double L)
{
    ...
    *epot = 0.0;
    ...
    for (int i = 0; i < 3 * (N - 1); i += 3) {
        ...
        int last_j = i+3;
        for (int j = i + 3; (3*N) - j >= 9; j += 9) {
            
            // double xj1 = rxyz[j + 0];
            // double yj1 = rxyz[j + 1];
            // double zj1 = rxyz[j + 2];
            __m256d xyz_j1 = _mm256_loadu_pd(&rxyz[j]);

            // double rx1 = xi - xj1;
            // double ry1 = yi - yj1;
            // double rz1 = zi - zj1;

            __m256d r_xyz1 = _mm256_sub_pd(xyz_i, xyz_j1);

            // rx1 = minimum_image(rx1, L);
            // ry1 = minimum_image(ry1, L);
            // rz1 = minimum_image(rz1, L);

            r_xyz1 = minimum_image(r_xyz1, L256);
            ...
            if (rij2_1 <= rcut2) {
                ...
                double fr = 24.0 * r2inv_1 * r6inv_1 * (2.0 * r6inv_1 - 1.0);
            ...
            // double yj2 = rxyz[j + 4];
            // double zj2 = rxyz[j + 5];
            __m256d xyz_j2 = _mm256_loadu_pd(&rxyz[j+3]);

            // double rx2 = xi - xj2;
            // double ry2 = yi - yj2;
            // double rz2 = zi - zj2;
            __m256d r_xyz2 = _mm256_sub_pd(xyz_i, xyz_j2);

            // rx2 = minimum_image(rx2, L);
            // ry2 = minimum_image(ry2, L);
            // rz2 = minimum_image(rz2, L);
            r_xyz2 = minimum_image(r_xyz2, L256);
            ...
            if (rij2_2 <= rcut2) {
                ...
                double fr = 24.0 * r2inv_2 * r6inv_2 * (2.0 * r6inv_2 - 1.0);
            ...
            // double yj3 = rxyz[j + 7];
            // double zj3 = rxyz[j + 8];
            __m256d xyz_j3 = _mm256_loadu_pd(&rxyz[j+6]);

            // double rx3 = xi - xj3;
            // double ry3 = yi - yj3;
            // double rz3 = zi - zj3;
            __m256d r_xyz3 = _mm256_sub_pd(xyz_i, xyz_j3);

            // rx3 = minimum_image(rx3, L);
            // ry3 = minimum_image(ry3, L);
            // rz3 = minimum_image(rz3, L);
            r_xyz3 = minimum_image(r_xyz3, L256);
            ...
            if (rij2_3 <= rcut2) {
                ...
                double fr = 24.0 * r2inv_3 * r6inv_3 * (2.0 * r6inv_3 - 1.0);
